...
class Publication {
  ...
  setContributors() {
    ...
    return this.source.agents.filter((agent) => {
      return agent.roles.filter(role => !otherRoles.includes(role)).length > 0;
    }).map((agent) => {
  ...
  setLinks() {
    this.links.push({
      ref: 'self',
      href: `${this.request.protocol}://${this.request.get('host')}/opds/work/${this.source.uuid}`,
